...
skynet_handle_register(struct skynet_context *ctx) {
	...
	for (;;) {
		int i;
		uint32_t handle = s->handle_index;         //1
		for (i=0;i<s->slot_size;i++,handle++) {
			if (handle > HANDLE_MASK) {            // 16777216
				// 0 is reserved
				handle = 1;
			}
			int hash = handle & (s->slot_size-1);    //1
			if (s->slot[hash] == NULL) {
				s->slot[hash] = ctx;
				s->handle_index = handle + 1; //2

				rwlock_wunlock(&s->lock);

				handle |= s->harbor;
				return handle;       //16777217
			}
		}
		assert((s->slot_size*2 - 1) <= HANDLE_MASK);   //16777216
...
_insert_name_before(struct handle_storage *s, char *name, uint32_t handle, int before) {
	if (s->name_count >= s->name_cap) {
		s->name_cap *= 2;
		assert(s->name_cap <= MAX_SLOT_SIZE);  //1073741824
...
skynet_handle_init(int harbor) {
	...
	s->harbor = (uint32_t) (harbor & 0xff) << HANDLE_REMOTE_SHIFT; //16777216
	s->handle_index = 1;
	s->name_cap = 2;
	s->name_count = 0;
